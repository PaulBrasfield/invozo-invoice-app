<template>
  <router-link
    class="invoice flex"
    :to="{ name: 'Invoice', params: { invoiceNumber: invoice.invoiceNumber } }"
  >
    <div class="left flex">
      <span class="tracking-number">#{{ invoice.invoiceNumber }}</span>
      <span class="due-date">{{ invoice.paymentDueDate }}</span>
      <span class="client">{{ invoice.clientName }}</span>
    </div>
